...
- `/create-card` — GPT destekli özel kart oluşturma (giriş gerektirir).
- `/api/cards/generate` — özel kart üretimi endpoint'i.
- `/terms`, `/privacy`, `/refund`, `/cookies`, `/subscriptions` — yasal sayfalar.
- `/api/ai-practice/chat`, `/api/ask/chat`, `/api/lemon-squeezy/webhook`, `/api/ai-practice/score`, `/api/ai-practice/translate`, `/api/quiz/validate-answer` — API route handler'ları.
...
- Migrations: `supabase/migrations/` altında sıralı SQL dosyaları:
  ...
  - `0015_custom_cards.sql`
  - `0016_add_create_card_usage_event.sql`
...
- `custom_cards` — kullanıcıların `/create-card` ile oluşturduğu özel kartlar (`source_key` ile `user_cards`/`practice_attempts` tarafından referanslanır).

RLS:

- `languages` public read verisidir (`anon` + `authenticated`).
- `user_profiles`, `user_cards`, `practice_attempts`, `custom_cards`, `ai_usage_events`, `user_subscriptions`, `ai_practice_scores` sadece satır sahibi tarafından okunur/yazılır (`auth.uid() = user_id`).
...
- Sadece Learn sayfasındaki `text-input` sorularda çalışır; `choice` sorularda çalışmaz.
- Kullanıcının yazılı cevabı önce yerel `isAnswerSimilarEnough` kontrolünden geçer; doğrudan doğruysa AI çağrılmaz.
- Doğrudan eşleşme başarısız olursa ve kullanıcı ücretsizse günlük 15 doğrulama limiti (`foxiesdeck:ai-quiz-validation:daily` localStorage key) uygulanır. Limit aşılınca veya timeout/API hatası olursa cevap sessizce yanlış kabul edilir.
- OpenAI çağrısı `POST /api/quiz/validate-answer` üzerinden yapılır; model `OPENAI_AI_PRACTICE_MODEL` (varsayılan `gpt-5-nano`), 3 saniye timeout, cevap sadece `t` (doğru) veya `y` (yanlış).
- Bu kota mevcut AI practice/ask limitlerinden (`ai_usage_events`) bağımsızdır; `ai_usage_events` tablosuna yazılmaz.
- İlgili dosyalar: `src/app/api/quiz/validate-answer/route.ts`, `src/features/quiz/ai-validate-answer.ts`, `src/features/quiz/use-ai-quiz-validation-limit.ts`, `src/features/quiz/components/quiz-station.tsx`.

### AI Kullanım Limitleri

`src/features/subscriptions/ai-usage-service.ts` tarafından `ai_usage_events` tablosundaki satırlar sayılarak uygulanır. Olay tipleri: `chat`, `translate`, `ask`, `create_card`.
