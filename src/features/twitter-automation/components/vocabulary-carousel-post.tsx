...
export function VocabularyCarouselPost({ card, nativeLanguage, presentation = "meaning", onSlideRef }: VocabularyCarouselPostProps) {
  ...
  const headlineLayout = presentation === "tier"
    ? "top-6 px-6 sm:top-8 sm:px-8"
    : "top-6 flex h-24 items-center justify-center px-6 sm:top-8 sm:h-28 sm:px-8";
  ...
  return (
    <article
      className="relative aspect-[3/4] w-[360px] shrink-0 overflow-hidden rounded-lg bg-[#16120f] p-5 text-[#fffaf4] sm:w-[440px] sm:p-6"
    ...
    >
      ...
      <h2 className={cn("absolute left-0 right-0 z-10 break-words text-center font-display text-4xl font-semibold leading-[0.92] sm:text-5xl", headlineLayout, usesSuperWater && "font-super-water")} style={{ color: presentation === "tier" ? TIER_ACCENTS[card.tier] : "#ffffff" }}>
        {displayHeadline}
      </h2>
