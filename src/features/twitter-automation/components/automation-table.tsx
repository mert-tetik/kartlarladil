...
export function AutomationTable({ onBack }: { onBack: () => void }) {
  ...
  const [oauthResult, setOauthResult] = useState<{ provider: "X" | "Instagram" | "Pinterest" | "YouTube"; result: string; errorCode?: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const instagramResult = search.get("instagramOAuth");
    const pinterestResult = search.get("pinterestOAuth");
    const youTubeResult = search.get("youtubeOAuth");
    if (!xResult && !instagramResult && !pinterestResult && !youTubeResult) return;

    const timeout = window.setTimeout(() => {
      setOauthResult(xResult
        ? { provider: "X", result: xResult, errorCode: search.get("xOAuthError") ?? undefined }
        : instagramResult
          ? { provider: "Instagram", result: instagramResult, errorCode: search.get("instagramOAuthError") ?? undefined }
          : pinterestResult
    ...
    search.delete("instagramOAuth");
    search.delete("instagramOAuthError");
