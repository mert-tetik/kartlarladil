...
const GENERATORS: Record<SelectableContentType, Array<{ value: string; label: string }>> = {
  ...
  video: [
    ...
    { value: "random-image-to-video", label: "Random Image to Video" },
    { value: "random-ai-video", label: "Random AI Video" },
...
export function AutomationTable({ onBack, onOpenSocialMedias }: { onBack: () => void; onOpenSocialMedias: () => void }) {
  ...
  const loadAutomation = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/twitter-automation/automations", { cache: "no-store" });
