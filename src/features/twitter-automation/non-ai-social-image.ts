...
function imageSvg(card: VocabularyCard, nativeLanguage: LanguageCode, mode: NonAiImageMode) {
  ...
  <text x="84" y="118" fill="#d9e4dc" font-family="Arial, sans-serif" font-size="30" font-weight="700" letter-spacing="3">${escapeXml(label)}</text>
  <rect x="84" y="202" width="912" height="676" rx="42" fill="#f8f4eb"/>
  <rect x="122" y="242" width="150" height="54" rx="27" fill="${color}"/>
  <text x="197" y="278" text-anchor="middle" fill="#fff" font-family="Arial, sans-serif" font-size="26" font-weight="700">${card.tier}</text>
  <text x="122" y="408" fill="#201d19" font-family="Georgia, serif" font-size="${termSize}" font-weight="700">${escapeXml(card.term)}</text>
  <line x1="122" y1="472" x2="894" y2="472" stroke="#d5cec3" stroke-width="3"/>
  <text x="122" y="558" fill="#5a514a" font-family="Arial, sans-serif" font-size="38">${escapeXml(meaning)}</text>
  <text x="122" y="652" fill="#292520" font-family="Arial, sans-serif" font-size="32">${escapeXml(card.examples[0]?.sentence ?? card.example)}</text>
  <text x="122" y="794" fill="${color}" font-family="Arial, sans-serif" font-size="30" font-weight="700">Learn it. Keep it. Use it.</text>
  <text x="84" y="964" fill="#c8d5cc" font-family="Arial, sans-serif" font-size="28">foxiesdeck.com</text>
</svg>`;
}
