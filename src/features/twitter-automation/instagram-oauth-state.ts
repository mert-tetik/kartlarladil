...
export const INSTAGRAM_OAUTH_STATE_COOKIE = "foxiesdeck:social-instagram-oauth-state";
...
export function createInstagramOAuthState(socialMediaId: number) {
  if (!Number.isSafeInteger(socialMediaId) || socialMediaId < 1) {
    throw new Error("A valid social media account is required.");
  }

  const data: InstagramOAuthState = {
    socialMediaId,
    state: crypto.randomBytes(32).toString("base64url"),
    expiresAt: Math.floor(Date.now() / 1000) + INSTAGRAM_OAUTH_STATE_MAX_AGE,
  };
...
export function readInstagramOAuthState(cookieValue: string | undefined) {
  ...
  try {
    const parsed = JSON.parse(Buffer.from(encodedData, "base64url").toString("utf8")) as Partial<InstagramOAuthState>;
...
export function hasMatchingInstagramOAuthState(savedState: InstagramOAuthState, returnedState: string | null) {
...
export const instagramOAuthStateCookieOptions = {
  ...
  path: "/api/twitter-automation/oauth/instagram",
