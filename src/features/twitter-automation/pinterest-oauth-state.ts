...
export const PINTEREST_OAUTH_STATE_COOKIE = "foxiesdeck:social-pinterest-oauth-state";
export const PINTEREST_OAUTH_STATE_MAX_AGE = 10 * 60;
...
export function createPinterestOAuthState(socialMediaId: number) {
  if (!Number.isSafeInteger(socialMediaId) || socialMediaId < 1) {
    throw new Error("A valid social media account is required.");
  }

  const data: PinterestOAuthState = {
    socialMediaId,
    state: crypto.randomBytes(32).toString("base64url"),
    expiresAt: Math.floor(Date.now() / 1000) + PINTEREST_OAUTH_STATE_MAX_AGE,
  };
...
export function readPinterestOAuthState(cookieValue: string | undefined) {
  ...
  try {
    const parsed = JSON.parse(Buffer.from(encodedData, "base64url").toString("utf8")) as Partial<PinterestOAuthState>;
...
export function hasMatchingPinterestOAuthState(savedState: PinterestOAuthState, returnedState: string | null) {
...
export const pinterestOAuthStateCookieOptions = {
  httpOnly: true,
  sameSite: "lax" as const,
  secure: process.env.NODE_ENV === "production",
  path: "/api/twitter-automation/oauth/pinterest",
  maxAge: PINTEREST_OAUTH_STATE_MAX_AGE,
};
