...
export function MobileCustomCardSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => setPreviewExpanded(true));
    const revealTimer = window.setTimeout(() => setPreviewRevealed(true), 520);
  ...
  return <div role="dialog" aria-modal="true" onPointerDown={(event) => { if (event.target === event.currentTarget) onClose(); }} className={`fixed inset-0 z-[71] flex flex-col justify-end bg-black/50 transition-opacity duration-300 lg:hidden ${entered ? "opacity-100" : "pointer-events-none opacity-0"}`}>
    <div className={`flex min-h-[62dvh] max-h-[92dvh] flex-col overflow-y-auto rounded-t-xl bg-background-card p-5 shadow-sm transition-transform duration-300 ease-out ${entered ? "translate-y-0" : "translate-y-full"}`} style={{ transform: entered ? `translateY(${dragY}px)` : undefined }}>
