...
export function MobileCustomCardSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <div role="dialog" aria-modal="true" onPointerDown={(event) => { if (event.target === event.currentTarget) onClose(); }} className={`fixed inset-0 z-[71] flex flex-col justify-end bg-black/50 transition-opacity duration-300 lg:hidden ${open ? "opacity-100" : "pointer-events-none opacity-0"}`}>
    <div onPointerDown={(event) => { dragStartY.current = event.clientY; event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={(event) => { if (dragStartY.current !== null) setDragY(Math.max(0, event.clientY - dragStartY.current)); }} onPointerUp={() => { if (dragY > 110) onClose(); setDragY(0); dragStartY.current = null; }} className={`min-h-[62dvh] max-h-[92dvh] overflow-y-auto rounded-t-xl bg-background-card p-5 shadow-sm transition-transform duration-300 ease-out ${open ? "translate-y-0" : "translate-y-full"}`} style={{ transform: open ? `translateY(${dragY}px)` : undefined }}>
